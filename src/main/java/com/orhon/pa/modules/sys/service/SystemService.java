...
//			systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void saveSummaryOffice(User user ) {
		if(user!=null && !user.equals("")) {
			SummaryOfficeRole role = new SummaryOfficeRole();
			role.setId(IdGen.uuid());
			role.setAsscociationOfficeId(user.getId());
			role.setDelFlag("0");
			role.setIdentificationSymbo(user.getProjectPorperty());
			userDao.summaryInsert(role);
		}
	}
	...
	public int getCount(String id) {
		Integer count = userDao.count(id);
		return count;
	}
