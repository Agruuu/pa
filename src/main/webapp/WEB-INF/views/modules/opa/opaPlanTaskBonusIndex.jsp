	...
	<script type="text/javascript">
		...
		function btnRender(row){
			...
			if((row.status == "${fns:getDictValue('申请中', 'opa_planTask_status', '')}")
					...
					str+="<shiro:hasPermission name='opa:opaPlanTask:bonus:index'><a href='${ctx}/opa/opaPlanTask/bonus/inputScore?id="+row.id 
...
<body>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.itemId}}" pId="{{pid}}">
				<td><a href="${ctx}/opa/opaPlanTaskBonus/view?id={{row.id}}">
					{{row.name}}
				</a></td>
				<td>
					{{row.content}}
