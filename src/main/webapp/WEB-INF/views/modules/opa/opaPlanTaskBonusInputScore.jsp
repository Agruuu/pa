...
<body>
	...
	<form:form id="inputForm" modelAttribute="opaPlanTask" action="${ctx}/opa/opaPlanTask/saveInputScore" method="post" class="form-horizontal">
		...
		<div id="content" class="form-search">
				<label id="inputScore">评分：<form:input path="score" htmlEscape="false" class="input-mini required number"/>&nbsp;&frasl;&nbsp;${opaPlanTask.value}分</label>
				<label id="inputResult">数值：<form:input path="result" htmlEscape="false" class="input-mini required number" onblur="calScore(this.value);"/>&nbsp;&frasl;&nbsp;${opaPlanTask.count}</label>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="opa:opaPlanTaskBonus:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="评 分"/>&nbsp;</shiro:hasPermission>
		</div>
	</form:form>
